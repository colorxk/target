...
public class HttpProxy extends Http {
	...
	private boolean response(){
		try {
			...
			for(String s:list)	//将list（request数据包转发给目标服务器）
				w.println(s);	
			//TODO w.print(s+"\r\n");
	...
	private String[] getFirst(String requestLine){
		...
		splits[0] = requestLine.substring(0, i);//获取method
		j = requestLine.indexOf(" ",++i);
		splits[5] = requestLine.substring(j+1);//获取version
	...
	public static String[] getdp(String url){
		...
		if(i>0 && url.charAt(i-1)==':' && i+1<url.length() && url.charAt(i+1)=='/'){
			dp[0] = url.substring(0,i-1);//获取http
			j = url.indexOf('/',i+=2);
			if(j>0){
				dp[1] = url.substring(i, j);//获取domain
				dp[2] = url.substring(j);//获取path
			}
			else{
